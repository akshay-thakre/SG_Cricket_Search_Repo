...
import { aggregateStaticPlayerStats, mergeWithLiveStats } from '../utils/aggregatePlayerStats';
...
// ── All-leagues aggregated panel ──────────────────────────────────────────────

function fmtN(v, decimals = 2) {
  if (v === null || v === undefined) return '—';
  const num = Number(v);
  return isNaN(num) ? '—' : decimals > 0 ? num.toFixed(decimals).replace(/\.?0+$/, '') : String(Math.round(num));
}

function AggStatBox({ label, value, accent = false }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: '14px', fontWeight: '700', color: accent ? '#1d4ed8' : '#1e293b' }}>
        {value !== null && value !== undefined ? String(value) : '—'}
      </div>
      <div style={{ fontSize: '9px', color: '#94a3b8', fontWeight: '600', marginTop: '2px', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
        {label}
      </div>
    </div>
  );
...
function AllLeaguesPanel({ results }) {
  ...
  const mergeLive = useCallback((liveStats) => {
    if (!liveStats || liveStatsMerged) return;
    setAgg((prev) => mergeWithLiveStats(prev, liveStats));
    setLiveStatsMerged(true);
  }, [liveStatsMerged]);

  // Expose merge function via a custom event so PlayerCard can notify us
  useEffect(() => {
    const handler = (e) => mergeLive(e.detail);
    window.addEventListener('cricsearch:liveStatsLoaded', handler);
    return () => window.removeEventListener('cricsearch:liveStatsLoaded', handler);
  }, [mergeLive]);

  const { batting: b, bowling: bwl, leaguesContributed } = agg;
  const hasBatting = b && (b.runs > 0 || b.innings > 0);
  const hasBowling = bwl && (bwl.wickets > 0 || bwl.balls > 0);
  ...
  return (
    <div style={{
      ...
      borderRadius: '12px',
      overflow: 'hidden',
      boxShadow: '0 2px 10px rgba(0, 66, 204, 0.10)',
      marginBottom: '1.5rem',
    }}>
      {/* Header */}
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: '11px', color: '#93c5fd', marginTop: '2px' }}>
            Combined totals · derived stats recalculated from raw counts
          </div>
        </div>
        <div style={{ display: 'flex', gap: '0.4rem', flexWrap: 'wrap' }}>
          {leaguesContributed.map((lg) => (
            <span key={lg} style={{
              backgroundColor: 'rgba(255,255,255,0.15)', color: '#e0f2fe',
      ...
      <div style={{ padding: '1rem 1.25rem' }}>
        ...
        <div style={{
          ...
          gap: '0.5rem',
          marginBottom: (hasBatting || hasBowling) ? '0.75rem' : 0,
        }}>
          {hasBatting && <>
            <AggStatBox label="Runs"    value={b.runs}            accent />
            <AggStatBox label="Innings" value={b.innings}         accent />
            <AggStatBox label="Bat Avg" value={fmtN(b.average)}   accent />
            <AggStatBox label="Bat SR"  value={fmtN(b.strikeRate)} accent />
          </>}
          {hasBowling && <>
            <AggStatBox label="Wickets" value={bwl.wickets}           accent />
            <AggStatBox label="Overs"   value={bwl.overs}             accent />
            <AggStatBox label="Bwl Avg" value={fmtN(bwl.average)}     accent />
            <AggStatBox label="Economy" value={fmtN(bwl.economy)}     accent />
          </>}
        </div>

        {/* Batting detail */}
        {hasBatting && (
          <div style={{ marginTop: '0.5rem', borderTop: '1px solid #e8eef5', paddingTop: '0.5rem' }}>
            <button
            ...
            >
              <span style={{ display: 'inline-block', transition: 'transform 0.15s', transform: battingOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>▶</span>
              🏏 Full Batting
            </button>
            {battingOpen && (
              <div style={{ marginTop: '0.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(60px, 1fr))', gap: '0.4rem' }}>
                <AggStatBox label="Matches" value={b.matches} />
                <AggStatBox label="NO"      value={b.notOuts} />
                <AggStatBox label="Balls"   value={b.balls || '—'} />
                <AggStatBox label="HS"      value={b.highestScore || '—'} />
                <AggStatBox label="50s"     value={b.fifties} />
                <AggStatBox label="100s"    value={b.hundreds} />
                <AggStatBox label="4s"      value={b.fours} />
                <AggStatBox label="6s"      value={b.sixes} />
                {b.ducks > 0 && <AggStatBox label="Ducks" value={b.ducks} />}
              </div>
            )}
          </div>
        )}
        ...
        {hasBowling && (
          <div style={{ marginTop: '0.5rem', borderTop: '1px solid #e8eef5', paddingTop: '0.5rem' }}>
            ...
            >
              <span style={{ display: 'inline-block', transition: 'transform 0.15s', transform: bowlingOpen ? 'rotate(90deg)' : 'rotate(0deg)' }}>▶</span>
              ⚡ Full Bowling
            </button>
            {bowlingOpen && (
              <div style={{ marginTop: '0.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(60px, 1fr))', gap: '0.4rem' }}>
                <AggStatBox label="Matches" value={bwl.matches} />
                <AggStatBox label="Innings" value={bwl.innings} />
                <AggStatBox label="Balls"   value={bwl.balls} />
                <AggStatBox label="Runs"    value={bwl.runs} />
                <AggStatBox label="Mdns"    value={bwl.maidens} />
                <AggStatBox label="Bwl SR"  value={fmtN(bwl.strikeRate)} />
                {bwl.bestBowling && <AggStatBox label="Best" value={bwl.bestBowling} />}
              </div>
            )}
          </div>
        )}
      ...
      <div style={{
        ...
        Static league data only · SCA live stats not yet included in totals
      </div>
    </div>
  );
}
